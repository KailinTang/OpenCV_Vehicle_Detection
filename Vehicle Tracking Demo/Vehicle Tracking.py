...
# Filter out the result in false region
...
flag_0 = True # Only the detection frame(first frame or detection frame)
trackedCars = []
time = 0
frameNum = 0
...
while True:
    ...
    if not flag_0:    # Non-detection frames
        for (roi_hist, (x,y,w,h)) in trackedCars:
            hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
            dst = cv2.calcBackProject([hsv],[0],roi_hist,[0,180],1)

            ret1, track_window = cv2.meanShift(dst, (x, y, w, h), term_crit)    # Mean-shift tracking algorithm
